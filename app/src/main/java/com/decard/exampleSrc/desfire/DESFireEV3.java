package com.decard.exampleSrc.desfire;
...
import com.decard.exampleSrc.desfire.ev3.VersionInfo;
import com.decard.exampleSrc.desfire.util.AesCmac;
import com.decard.exampleSrc.desfire.util.CRC16;
...
import com.decard.exampleSrc.desfire.util.AES;
import com.decard.exampleSrc.desfire.util.CRC32;
...
public class DESFireEV3 extends DESFireEV1{

    private static final String TAG = DESFireEV3.class.getSimpleName();

    //private IsoDep isoDepIso;
    ...
    private byte[] SessionKey; // filled by authenticateAesEv2First
    ...
    private final byte CHANGE_KEY_EV2_SECURE_COMMAND = (byte) 0xC6;
    ...
    public static final byte[] ACCESS_RIGHTS_DEFAULT = Utils.hexStringToByteArray("1234"); // R&W access 1, CAR 2, R 3, W 4
    ...
    private final byte[] PADDING_FULL = Utils.hexStringToByteArray("80000000000000000000000000000000");
    ...
    public DesfireAuthenticateLegacy desfireD40;
    ...
    public DESFireEV3(IsoDepWrapper isoDep) {
        this.isoDep = isoDep;
        ...
        setAdapter(new DefaultIsoDepAdapter(isoDep,true));
    }
    ...
    public boolean test() {
        ...
        byte[] paddingReader = Utils.hexStringToByteArray("0000000000000000");
        ...
        return Arrays.equals(DecryptedResponseTest, DecryptedResponseExpTest);
    }
    ...
    public boolean createApplicationAes(byte[] applicationIdentifier, int numberOfApplicationKeys) {
        ...
        if (checkResponse(response)) {
            ...
            log(methodName, "FAILURE with " + Utils.printData("errorCode", errorCode));
            return false;
        }
    }
    ...
    public boolean getApplicationsIsoData() {
        ...
        for (int i = 0; i < appIdsList.size(); i++) {
            ...
            Log.d(TAG, "i: " + i + Utils.printData(" appId", appId));
            ...
            if (posAppId > 0) {
                ...
                Log.d(TAG, printData("appId", appIdTemp) + Utils.printData(" isoFileId", isoFileId) + Utils.printData(" dfName", dfName));
            }
        ...
        for (int i = 0; i < isoFileIdsList.size(); i++) {
            Log.d(TAG, "i: " + i + Utils.printData(" isoFileId", isoFileIdsList.get(i)));
            Log.d(TAG, "i: " + i + Utils.printData(" isoDfName", isoDfNamesList.get(i)));
        }
    ...
    private boolean createARecordFile(byte fileNumber, CommunicationSettings communicationSettings, byte[] accessRights, int recordSize, int maximumNumberOfRecords, boolean isLinearRecordFile) {
        ...
        // build the command string
        byte[] recordSizeBytes = Utils.intTo3ByteArrayInversed(recordSize);
        byte[] maximumNumberOfRecordsBytes = Utils.intTo3ByteArrayInversed(maximumNumberOfRecords);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        baos.write(fileNumber);
        baos.write(commSettings);
        baos.write(accessRights, 0, accessRights.length);
 ...
 */
    ...
    public boolean createATransactionMacFileFull(byte fileNumber, CommunicationSettings communicationSettings, int commitReaderIdAuthKeyNumber, int changeAccessRightsKeyNumber, int readAccessKeyNumber, boolean enableCommitReaderId, byte[] transactionMacKey) {
        ...
        byte[] padding1 = Utils.hexStringToByteArray("0000000000000000"); // 8 bytes
 ...
 */
    ...
    public boolean createTransactionMacFileEv2(byte fileNumber, byte[] transactionMacAccessRights, byte[] key) {
        ...
        log(methodName, "started", true);
        log(methodName, "fileNumber: " + fileNumber + Utils.printData(" TransactionMacKey", key));
        // sanity checks
    ...
    public byte[] readFromATransactionMacFile(byte fileNumber) {
        ...
        if (receivedData.length == 12) {
            ...
            Log.d(TAG, "TMAC counter: " + tmacInt + Utils.printData(" tmacEnc", tmacEnc));
...
padding add up to 16 bytes. As the data is always a multiple of 16 bytes, no padding is needed at the end of the TMI.
    ...
    private byte[] getSesTMMACKey(byte[] tmc, byte[] uid) {
        ...
        int tmcOld = Utils.intFrom4ByteArrayInversed(tmc);
        ...
        Log.d(TAG, "getSesTMMACKey " + Utils.printData("sv1", sv1));
        return sv1;
    }
    ...
    private byte[] readFromADataFileRawFull(byte fileNumber, int offset, int length) {
        ...
        byte[] commandCounterLsb2 = Utils.intTo2ByteArrayInversed(CmdCounter);
        byte[] padding = Utils.hexStringToByteArray("0000000000000000");
        byte[] startingIv = new byte[16];
    ...
    public int readFromAValueFile(byte fileNumber) {
        ...
        log(methodName, "commMode:" + commMode);
    ...
    private int readFromAValueFileRawMac(byte fileNumber) {
        ...
        if (verifyResponseMac(responseMACTruncatedReceived, data)) {
            ...
            errorCode = RESPONSE_OK.clone();
            errorCodeReason = methodName + " SUCCESS";
            return Utils.byteArrayLength4InversedToInt(data);
        } else {
            log(methodName, methodName + " FAILURE");
            errorCode = RESPONSE_FAILURE.clone();
            errorCodeReason = methodName + " FAILURE";
    ...
    private int readFromAValueFileRawFull(byte fileNumber) {
        ...
        byte[] padding = Utils.hexStringToByteArray("0000000000000000"); // fixed 8 bytes
        byte[] startingIv = new byte[16];
        ByteArrayOutputStream decryptBaos = new ByteArrayOutputStream();
        ...
        if (verifyResponseMac(responseMACTruncatedReceived, encryptedData)) {
            ...
            errorCode = RESPONSE_OK.clone();
            errorCodeReason = methodName + " SUCCESS";
            return Utils.byteArrayLength4InversedToInt(decryptedData);
        } else {
            log(methodName, methodName + " FAILURE");
            errorCode = RESPONSE_FAILURE.clone();
            errorCodeReason = methodName + " FAILURE";
    ...
    private boolean changeAValueFileRawFull(byte fileNumber, int changeValue, boolean isCredit) {
        ...
        byte[] value = Utils.intTo4ByteArrayInversed(changeValue);
        log(methodName, printData("value", value));
        byte[] padding2 = Utils.hexStringToByteArray("800000000000000000000000"); // 12 bytes
        ...
        try {
            ...
            log(methodName, Utils.printData("apdu", apdu));
            response = isoDep.transceive(apdu);
    ...
    private boolean commitReaderIdFull() {
        ...
        log(methodName, printData("transactionMacReaderId", transactionMacReaderId));
        //byte[] READER_ID = Utils.hexStringToByteArray("28BF1982BE086FBC60A22DAEB66613EE"); // 16 bytes
        ...
        byte[] commandCounterLsb2Reader = Utils.intTo2ByteArrayInversed(CmdCounter);
        byte[] paddingReader = Utils.hexStringToByteArray("0000000000000000");
        byte[] startingIvReader = new byte[16];
        ByteArrayOutputStream decryptBaosReader = new ByteArrayOutputStream();
        decryptBaosReader.write(IV_LABEL_DEC, 0, IV_LABEL_DEC.length); // (byte) 0x5A, (byte) 0xA5
    ...
    public boolean commitTMACTransactionEv2() {
        ...
        byte[] commandCounterLsb3Reader = Utils.intTo2ByteArrayInversed(CmdCounter);
        ...
        if (fullResponseDataLength > 8) {
            ...
            int tmcInt = Utils.byteArrayLength4InversedToInt(tmcByte);
 ...
 */
        ...
        byte[] transactionTimerNumberOfSecondsBytes = Utils.intTo4ByteArrayInversed(transactionTimerNumberOfSeconds);
        //byte[] transactionTimerNumberOfSecondsBytes = Utils.intTo3ByteArrayInversed(transactionTimerNumberOfSeconds);
        ...
        //byte[] data = Utils.hexStringToByteArray("0800000000E80300000000000001"); // 12 bytes, first 08 is fileNumber
    ...
    public FileSettings[] getAllFileSettings() {
        ...
        for (int i = 0; i < numberOfFileIds; i++) {
            ...
            Log.d(TAG, "i: " + i + Utils.printData(" fileSettingsByte", fileSettingsByte));
            Log.d(TAG, Utils.printData("errorCode", errorCode));
    ...
    public byte[] getFileSettings(byte fileNumber) {
        ...
        if (checkResponse(response)) {
            Log.d(TAG, "response SUCCESS");
            Log.d(TAG, "return for fileNumber " + fileNumber + " : " + Utils.printData("responseData", responseData));
            System.arraycopy(RESPONSE_OK, 0, errorCode, 0, RESPONSE_OK.length);
            return responseData;
        } else {
            Log.d(TAG, "response FAILURE");
...
fileSize: 128
        ...
        // status: working !
        //commandData = Utils.hexStringToByteArray("4000e0d1f1212a00004f00004f0000200000750000");
        //log(methodName, printData("commandData", commandData));

        // todo this is manually added by NdefForSdm value test 11 = NO encrypted PICC data but encrypted file data
        //commandData = Utils.hexStringToByteArray("4000e011f1f12500002500002000004b0000");

        // test 12 - encrypted PICC data, encrypted File data, sdm keys for all is 1, working
        //commandData = Utils.hexStringToByteArray("4000e0d1f1112a00004f00004f0000200000750000");
...
fileSize: 128
    ...
    public boolean authenticateAesEv2First(byte keyNumber, byte[] key) {
        ...
        boolean debug = true; // if true each single step is print out for debugging purposes
        logData = "";
        invalidateAllData();
    ...
    private byte[] getSessionKeyAes(byte[] rndA, byte[] rndB) {
        log("getSessionKeyAes", Utils.printData("rndA", rndA) + Utils.printData(" rndB", rndB), true);
    ...
    public boolean authenticateAesLegacy(byte keyNumber, byte[] key) {
        ...
        if (rndAEqual) {
            ...
            */
            ...
            desfireD40.SessionKey = SessionKey.clone();
            desfireD40.keyNumberUsedForAuthentication = keyNumber;
            log(methodName, "*** AUTHENTICATED ***");
    ...
    public boolean changeApplicationKeyFull(byte keyNumber, byte keyVersion, byte[] keyNew, byte[] keyOld) {
        ...
        if (keyNumberUsedForAuthentication != keyNumber) {
            ...
            byte[] padding = Utils.hexStringToByteArray("8000000000000000000000");
            ByteArrayOutputStream baosData = new ByteArrayOutputStream();
            ...
            byte[] padding = Utils.hexStringToByteArray("800000000000000000000000000000");
            ByteArrayOutputStream baosData = new ByteArrayOutputStream();
    ...
    public boolean changeApplicationKeyToDesFull(byte keyNumber, byte keyVersion, byte[] keyNew, byte[] keyOld) {
       ...
       // if (!checkAuthentication()) return false;
        ...
        if (keyNumberUsedForAuthentication != keyNumber) {
            ...
            // this is for case 2, auth key number == key number to change
            //(New KeyValue || New KeyVersion || Padding)
            byte[] padding;
            if(keyNew.length == 8)
                padding = Utils.hexStringToByteArray("80000000000000");
            else
                padding = Utils.hexStringToByteArray("800000000000000000000000000000");
        ...
        // C40000BC354CD50180D40DB52D5D8CA136249A0A14154DBA1BE0D67C408AB24CF0F3D3B4FE333C6A
        // C4 0000 BC354CD5 01 80D40DB52D5D8CA136249A0A14154DBA1BE0D67C408AB24CF0F3D3B4FE333C6A
        byte[] macInput = getMacInput(CHANGE_KEY_EV2_SECURE_COMMAND, new byte[]{(byte)(keyNumber)}, encryptedData);
        log(methodName, printData("macInput", macInput));
        ...
        // Data (CmdHeader = keyNumber || Encrypted Data || MAC)
        ByteArrayOutputStream baosChangeKeyCommand = new ByteArrayOutputStream();
        baosChangeKeyCommand.write((byte)(keyNumber|0x80));
        ...
        byte[] responseMACTruncatedReceived;
        try {
            apdu = wrapMessage(CHANGE_KEY_EV2_SECURE_COMMAND, changeKeyCommand);
            log(methodName, printData("apdu", apdu));
            response = isoDep.transceive(apdu);
            log(methodName, printData("response", response));
            //Log.d(TAG, methodName + printData(" response", response));
    ...
    public boolean changeApplicationKeyToDes(byte keyNumber, byte keyVersion, byte[] keyNew, byte[] keyOld) {
        final String methodName = "changeApplicationKeyToDes";
        ...
        if ((keyNew == null) || (keyNew.length != 8)) {
        ...
        if ((SessionKey == null) || (SessionKey.length != 16)) {
            log(methodName,methodName + " SESSION_KEY_AES is null or not of length 8 (missing auth ?), aborted");
            Log.e(TAG, methodName + " SESSION_KEY_AES is null or not of length 8 (missing auth ?), aborted");
            System.arraycopy(RESPONSE_FAILURE_MISSING_AUTHENTICATION, 0, errorCode, 0, 2);
            return false;
        }
        ...
        // altered.
        log(methodName, printData("new key before setKeyVersion", keyNew));
        byte KEY_VERSION = 0;
        desfireD40.setKeyVersion(keyOld, 0, keyOld.length, KEY_VERSION);
        desfireD40.setKeyVersion(keyNew, 0, keyNew.length, KEY_VERSION);
        System.out.println(printData("keyOld", keyOld));
        System.out.println(printData("keyNew", keyNew));
        log(methodName,printData("new key after  setKeyVersion", keyNew));

        byte[] plaintext = new byte[32]; // this is the final array
        int nklen = 16;
        System.arraycopy(keyNew, 0, plaintext, 0, keyNew.length);
        log(methodName,printData("plaintext", plaintext));
        // 8-byte DES keys accepted: internally have to be handled w/ 16 bytes
        System.arraycopy(keyNew, 0, plaintext, 8, keyNew.length);
        keyNew = Arrays.copyOfRange(plaintext, 0, 16);
        System.out.println("*** 3");
        log(methodName,printData("newKey TDES", keyNew));

        // xor the new key with the old key if a key is changed different to authentication key
        if ((keyNumber & 0x0F) != keyNumberUsedForLegacyAuthentication) {
            for (int i = 0; i < keyNew.length; i++) {
                plaintext[i] ^= keyOld[i % keyOld.length];
            }
        }
        log(methodName,printData("plaintext", plaintext));
        ...
        System.out.println(printData("SESSION_KEY_AES", SessionKey));
       // ciphertext = decrypt(SessionKey, plaintext);
        byte[] iv = new byte[16];
        byte[] ciphertext = AES.decrypt(iv,SessionKey,plaintext);
        ...
        try {
            ...
            System.out.println("*** 7 " + Utils.printData("changeKeyDesResponse", changeKeyDesResponse));
...
*/
    ...
    public boolean formatPicc(int keyType) {
        logData = "\n";
        ...
        log(methodName, " selectApplicationByAid");
        ...
        log(methodName, " selectApplicationByAid success");
        ...
        if(keyType == 0)
            success = desfireD40.authenticateD40(MASTER_APPLICATION_KEY_NUMBER, MASTER_APPLICATION_KEY_DES_DEFAULT);
        else
            //success = authenticateAesEv2First(MASTER_APPLICATION_KEY_NUMBER,MASTER_APPLICATION_KEY_AES_DEFAULT);
            success = authenticateAesLegacy(MASTER_APPLICATION_KEY_NUMBER,MASTER_APPLICATION_KEY_AES_DEFAULT);
        ...
        log(methodName, "authenticateD40/authenticateAesEv2First sucess");
        ...
        try {
            ...
            log(methodName,  printData("wrappedCommand", wrappedCommand));
    ...
    public ApplicationKeySettings getApplicationKeySettings() {
        if (!checkIsoDep()) return null;
        byte[] keySettings = getKeyEV3Settings();
     ...
     * Note: depending on the application master key settings this requires a preceding authentication
     * with the application master key
     * This is called from getKeyEV3Settings after successful checkAuthentication
     * @return a 2 byte array on success or null on failure
     */
    ...
    private byte[] sendData(byte[] apdu) throws IOException {
    ...
    public boolean changeAesKeyToDes(byte authenticationKeyNumber, byte changeKeyNumber,
        ...
        if ((changeKeyOld == null) || (changeKeyOld.length != 16)) {
            Log.e(TAG, methodName + " changeKeyOld is NULL or of wrong length, aborted");
            System.arraycopy(RESPONSE_FAILURE, 0, errorCode, 0, 2);
            return false;
        }
        ...
        if ((SesAuthENCKey == null) || (SesAuthENCKey.length != 16)) {
            log(methodName,methodName + " SesAuthENCKey is null or not of length 8 (missing auth ?), aborted");
            Log.e(TAG, methodName + " SesAuthENCKey is null or not of length 8 (missing auth ?), aborted");
            System.arraycopy(RESPONSE_FAILURE_MISSING_AUTHENTICATION, 0, errorCode, 0, 2);
            return false;
        }
        System.out.println("*** 2");
        ...
        byte KEY_VERSION = 0;
        desfireD40.setKeyVersion(changeKeyOld, 0, changeKeyOld.length, KEY_VERSION);
        desfireD40.setKeyVersion(changeKeyNew, 0, changeKeyNew.length, KEY_VERSION);
        ...
        System.out.println(printData("SESSION_KEY_AES", SesAuthENCKey));
        ciphertext = AES.encrypt(startingIv, SesAuthENCKey, plaintext);

        Log.d(methodName, printData("AES ciphertext after decryption", ciphertext));
        System.out.println("*** 5");
        byte[] apdu = new byte[5 + 1 + ciphertext.length + 1];
        apdu[0] = (byte) 0x90;
        apdu[1] = CHANGE_KEY_COMMAND;
        ...
        try {
            //response = isoDep.transceive(wrapMessage(selectApplicationCommand, applicationIdentifier));
            changeKeyDesResponse = isoDep.transceive(apdu);
            log(methodName, printData("changeKey Aes2Des Response", changeKeyDesResponse));
            System.arraycopy(returnStatusBytes(changeKeyDesResponse), 0, errorCode, 0, 2);
            //System.arraycopy(selectApplicationResponse, 0, response, 0, selectApplicationResponse.length);
            System.out.println("*** 7 " + Utils.printData("changeKey Aes2Des Response", changeKeyDesResponse));
            if (checkResponse(changeKeyDesResponse)) {
                return true;
            } else {
                return false;
            ...
            //throw new RuntimeException(e);
            log(methodName, "changeKey Aes2Des transceive failed: " + e.getMessage());
            byte[] responseManual = new byte[]{(byte) 0x91, (byte) 0xFF};
            System.arraycopy(responseManual, 0, e, 0, 2);
            return false;
        }
