...
public class DefaultIsoDepAdapter implements IsoDepAdapter {
	...
	private static final String TAG = DefaultIsoDepAdapter.class.getSimpleName();
	...
	public byte[] transceive(byte[] request) throws IOException {
		...
		if(logging) {
			Log.d(TAG, "<=== " + Utils.getHexString(response, true) + " (" + response.length + ")");
		}
    ...
    public byte[] sendCommandChain(byte command, byte[] parameters, int offset, int length) throws Exception {
        ...
        while(true) {
            ...
            Log.d(TAG, "request ===> " + Utils.getHexString(request, true));
            byte[] response = transceive(request);
            Log.d(TAG, "response ===> " + Utils.getHexString(response, true));

            count += nextLength;

            if (response[response.length - 2] != (byte) 0x91) {
                Log.d(TAG, "Invalid response " + String.format("%02x", response[response.length - 2] & 0xff));
            ...
            Log.d(TAG, "output1 <=== " + Utils.getHexString(output.toByteArray(), true));

            byte status = response[response.length - 1];

            if (status == OPERATION_OK) {
                if(count != length) {
                    Log.d(TAG,"Expected sent " + length + " bytes but was " + count);
                    throw new IllegalArgumentException("Expected sent " + length + " bytes but was " + count);
                }
                output.write(response,response.length - 2,2);
                break;
            } else if (status == ADDITIONAL_FRAME) {
                nextCommand = ADDITIONAL_FRAME;
            } else {
                Log.d(TAG,"PICC error code: " + Integer.toHexString(status & 0xFF));
                throw new Exception("PICC error code: " + Integer.toHexString(status & 0xFF));
            }
        }
        Log.d(TAG, "output <=== " + Utils.getHexString(output.toByteArray(), true));
