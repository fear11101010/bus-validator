package com.decard.exampleSrc.desfire.ev3;
...
import com.decard.exampleSrc.desfire.util.TripleDES;
...
public class DesfireAuthenticateLegacy {
    ...
    private static final String TAG = DesfireAuthenticateLegacy.class.getSimpleName();
    ...
    public byte keyNumberUsedForAuthentication = -1;
    ...
    public DesfireAuthenticateLegacy(IsoDepWrapper isoDep, boolean printToLog) {
        this.isoDep = isoDep;
    ...
    private byte[] decrypt(byte[] key, byte[] data) {
        ...
        log(logString, printData("key", key) + Utils.printData(" data", data), true);
    ...
    public boolean changeAesKeyToDes(byte authenticationKeyNumber, byte changeKeyNumber,
        ...
        // xor the new key with the old key if a key is changed different to authentication key
        log(methodName,"keyNumberUsedForAuthentication:" + keyNumberUsedForAuthentication);
         ...
         */
        byte[] crc32 = CRC32.get(changeKeyNew);
        System.arraycopy(crc32, 0, plaintext, nklen , 4);
        ...
        //ciphertext = decrypt(SessionKey, plaintext);
        byte[] iv = new byte[16];
        ciphertext = AES.decrypt(iv,SessionKey,plaintext);
        ...
        apdu[1] = CHANGE_KEY_COMMAND;
        apdu[4] = (byte) (1 + plaintext.length);
        apdu[5] = (byte)(changeKeyNumber|0x00);
        System.arraycopy(ciphertext, 0, apdu, 6, ciphertext.length);
        Log.d(methodName, printData("apdu", apdu));
        System.out.println("*** 6");
        byte[] changeKeyDesResponse = new byte[0];
    ...
    public boolean authenticateD40(byte keyNo, byte[] key) {
        ...
        try {
            ...
            log(methodName, "- send auth apdu   " + Utils.printData("apdu    ", apdu));
            response = isoDep.transceive(apdu);
            log(methodName, "- receive response " + Utils.printData("response", response));
        } catch (IOException e) {
            Log.e(TAG, methodName + " transceive failed, IOException:\n" + e.getMessage());
            log(methodName, "IOException: " + e.getMessage());
            System.arraycopy(RESPONSE_FAILURE, 0, errorCode, 0, 2);
        ...
        byte[] iv0 = new byte[8];
        log(methodName, "step 06 decrypt the encRndB using TripleDES.decrypt with key " + Utils.printData("key", key) + Utils.printData(" iv0", iv0));
        ...
        log(methodName, "data before XORing " + Utils.printData("data", rndArndBLeftRotated) + Utils.printData(" cipheredBlock", cipheredBlock));
        ...
        for (int i = 0; i < rndArndBLeftRotated.length; i += 8) {
            ...
            log(methodName, "TripleDES.decrypt " + Utils.printData("cipheredBlock", cipheredBlock));
        ...
        log(methodName, Utils.printData("- encrypted rndA || rndB left rotated", encryptedRndArndBLeftRotated));
        ...
        log(methodName, "step 13 decrypt the encrypted rndA left rotated using TripleDES.decrypt with key " + Utils.printData("key", key) + Utils.printData(" iv0", iv0));
    ...
    public byte[] tripleDesSendModeDecryption(byte[] tdesKeyExt, byte[] ciphertextExt) {
        ...
        byte[] ct1XoredDecrypted = TripleDES.decrypt(tdesKey, ct1Xored, 0, 8);
        ...
        byte[] ct2XoredDecrypted = TripleDES.decrypt(tdesKey, ct2Xored, 0, 8);
     ...
     */
    // source: nfcjLib
    public void setKeyVersion(byte[] a, int offset, int length, byte version) {
        log("setKeyVersion", Utils.printData("a", a) + " offset: " + offset + " length: " + length + " version: " + version, true);
        //Log.d(TAG, "setKeyVersion " + Utils.printData("a", a) + " offset: " + offset + " length: " + length + " version: " + version);
    ...
    private byte[] getRandomData(byte[] key) {
        ...
        //Log.d(TAG, "getRandomData " + Utils.printData("var", var));
    ...
    private byte[] getSessionKeyDes(byte[] rndA, byte[] rndB) {
        log("getSessionKey", Utils.printData("rndA", rndA) + Utils.printData(" rndB", rndB), true);
    ...
    private byte[] getData(byte[] responseAPDU) {
        log("getData", Utils.printData("responseAPDU", responseAPDU), true);
        //Log.d(TAG, "getData " + Utils.printData("responseAPDU", responseAPDU));
