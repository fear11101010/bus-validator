...
public class RideAndAlight {
    ...
    private final String ALIGHT_AND_REFUND_OF_SV_NOT_NEGATIVE = "D630";
    private final String ALIGHT_AND_REFUND_OF_SV_NEGATIVE = "D730";
    ...
    public RideAndAlight(@NonNull ReadWriteInCard readWriteInCard) {
        try {
    ...
    private static class Ride {
        ...
        private void updateAttributeInfo() {
            if (attributeInfo != null) {
                ...
                attributeInfo.setTxnDataId(Utils.hexToByte(String.format("%04X", transId)));
                if (isNegative) {
                    int maxFare = station.getMaxFare();
                    int negativeValue = Math.abs(maxFare - Utils.byteToInteger(attributeInfo.getNegativeValue()));
                    attributeInfo.setNegativeValue(Utils.hexToByte(String.format("%04X", negativeValue)));
                }
            }
        ...
        public void setStation(Route.Station station) {
            ...
            this.isNegative = Utils.charArrayToIntLE(ePurseInfo.getBinRemainingSV(), 4) - station.getMaxFare() < 0;
        }
...
//            ePurseInfo.setBinRemainingSV(Utils.hexToByte(String.format("%04X",sv-maxFare)));
                ePurseInfo.setBinRemainingSV(Utils.intToCharArrayLE(Math.max(sv - maxFare, 0)));
//            ePurseInfo.setBinCashbackData(Utils.hexToByte(String.format("%04X",maxFare)));
                ePurseInfo.setBinCashbackData(Utils.intToCharArrayLE(maxFare));
            }
        }
        ...
        private void updateStoredValueLog() {
            ...
            String time = date.get("hour") + date.get("minute") + "00000";
            storedLogInformation.setTime(Utils.hexToByte(String.format("%02X", Integer.parseInt(time, 2)))[0]);
            storedLogInformation.setPlace1(Utils.hexToByte(String.format("%02x", Integer.parseInt(station.getCode().substring(0, 3))) + String.format("%02x", Integer.parseInt(station.getCode().substring(3)))));
            storedLogInformation.setPlace2(Utils.hexToByte("0000"));
            storedLogInformation.setStoredValueLogId(ePurseInfo.getBinExecutionId());
    ...
    private class Alight {
        ...
        public void setStation(Route.Station station) {
            this.station = station;
            this.isNegative = station.getFare()[RideAndAlight.this.getRide().getStation().getPosition()] - Utils.charArrayToIntLE(ePurseInfo.getBinCashbackData(), 4) < 0;
        }
        ...
        private void updateAttributeInfo() {
            if (attributeInfo != null) {
                ...
                attributeInfo.setTxnDataId(Utils.hexToByte(String.format("%04X", transId)));
                if (isNegative) {
                    int fare = station.getFare()[RideAndAlight.this.getRide().getStation().getPosition()];
                    int negativeValue = Math.abs(fare - Utils.charArrayToIntLE(ePurseInfo.getBinCashbackData(),4));
                    attributeInfo.setNegativeValue(Utils.hexToByte(String.format("%04X", negativeValue)));
                }
                else {
...
//            ePurseInfo.setBinCashbackData(Utils.hexToByte(String.format("%04X",maxFare)));
        ...
        private void updateStoredValueLog() {
            ...
            String time = date.get("hour") + date.get("minute") + "00000";
            storedLogInformation.setTime(Utils.hexToByte(String.format("%02X", Integer.parseInt(time, 2)))[0]);
            storedLogInformation.setPlace1(Utils.hexToByte(String.format("%02x", Integer.parseInt(RideAndAlight.this.getRide().getStation().getCode().substring(0, 3))) + String.format("%02x", Integer.parseInt(RideAndAlight.this.getRide().getStation().getCode().substring(3)))));
            storedLogInformation.setPlace2(Utils.hexToByte(String.format("%02x", Integer.parseInt(station.getCode().substring(0, 3))) + String.format("%02x", Integer.parseInt(station.getCode().substring(3)))));
            storedLogInformation.setStoredValueLogId(ePurseInfo.getBinExecutionId());
        ...
        private void updateAccessLogInformation() {
            ...
            int maxFare = station.getMaxFare();
            gateAccessLogInformation.setAmountOfDistanceFare(Utils.hexToByte(String.format("%06X", station.getFare()[RideAndAlight.this.getRide().getStation().getPosition()]-maxFare)));
        }
